...
public class PowerDbaManager {
    ...
    private static String INITIAL_PAGE_STRING = "DB_SESSIONS";
    private static String OBJECT_NAME = "PowerDbaManager";
    
    private PowerDbaView myView;
    ...
    public PowerDbaManager(JspEnvironment env) {
      this.env = env;
      this.myView = new PowerDbaView(env);
    }

    // process method returns the view.
    public PowerDbaView process() throws Exception {
      ...
      try {
        ...
        if ( actionString.equals("0") || actionString.equals("1") ) {
        	action = INITIAL_PAGE; 
          actionString = INITIAL_PAGE_STRING;
        }
        ...
        setDatabaseList(env.getParameter("database"), this.database==null?null:this.database.getDatabase());
        ...
        // Is this an xml defined Action/Query
        if ( ActionHolder.getList().contains(this.actionString) && connected ) {
	        	...
	        	// We override the isRAC that is set in the action xml based on the fact that the jsp page is foing
	        	// all instances, but ONLY where it is set true in the action object.
	        	//boolean masterIsRac = 
	          //	(this.allInstances.equals("Y")&&actionObj.isRac())?true:false;
	        	
	        	boolean masterIsRac = false;
	        	if (this.allInstances.equals("U")) {
	        		masterIsRac = actionObj.isRac();
	        	} else  if  ( this.allInstances.equals("N")) {
	        		masterIsRac = false;
	        	} else 	if ( this.allInstances.equals("Y")) {
	        		masterIsRac = true;
	        	}
	          ...
	          // Run each of the queries associated with the action.
	        	...
	        	for ( int i=0; i<actionObj.getQueries().size(); i++ ) {
	        		//System.out.println(actionObj==null?"actionObj is null":"actionObj not null");
	        		//System.out.println(actionObj.getQueries()==null?"getQueries is null":"getQueries not null");
	        		//System.out.println((String)actionObj.getQueries().get(i)==null?"String is null":"String not null");
        ...
        switch (action) {
          ...
          case PowerDbaActions.CHARTS_ACTIVE_SESSIONS:          
            myView.setTitle("Active Sessions");
            myView.setMenus(MenuGenerator.CHARTS_MENU);         
            setChartGroup("CHARTS_ACTIVE_SESSIONS");
            ChartHolder.cleanCharts(1);
            break;
          ...
          case PowerDbaActions.CHARTS_ACTIVE_SESSIONSBYSERVICE:
            myView.setTitle("Current Active Sessions");
            myView.setMenus(MenuGenerator.CHARTS_MENU);         
            setChartGroup("CHARTS_ACTIVE_SESSIONSBYSERVICE");
            ChartHolder.cleanCharts(1);
            break;
          ...
          case PowerDbaActions.DB_SESS_ZOOM:
            ...
            if ( validateSession(sid) ) {   
	            ...
	            if ( q != null ) {
	              ...
	              q.setDatabase(database);
	              sb.append(q.getHtml(ProcessDAO.DETAIL,7));
	            }
	            ...
	            sb.append(ProcessDAO.buildCurrentSql(sql, sid, database, "Current SQL", sa.getHashValue(), 8));
	            sb.append("</td>");   
	            ...
	            sb.append("<td><img src=images/clear.gif width=10 height=1></td>");
	            sb.append("<td valign=top>");
	            q = QueryHolder.getQuery(database, env, "sessiontime");
	            if ( q != null ) {
	              q.setBindValue("sid", Long.toString(sid));
	              q.setDatabase(database);
	            ...
	            sb.append(DIV);	            
	            //sb.append(ProcessDAO.buildCurrentSql(sql, sid, database, "Current SQL", sa.getHashValue(), 3));
      ...
      case PowerDbaActions.DB_SESSION_STATS:
        ...
        break;
          ...
          case PowerDbaActions.EXPLAIN_PLAN_FROM_MEMORY:
            myView.setMenu1(MenuGenerator.INSTANCE_MENU);
            ...
            if ( env.getParameter("sqltoexplain").equals("") || env.getParameter("sqltoexplain").equals("null") ) {
              sqlId = env.getParameter("sqlid");
              childNumber = env.getParameter("childnumber");
              //myView.setHash(Long.toString(hashValue));
              sqlToExplain = ProcessDAO.getExplainOutputFromSqlId(sqlId, database);
              myView.setTitle("Explain Plan for SQL_ID: " + sqlId + " Child#: " + childNumber);
            } else {
            ...
            myView.setHtml(ProcessDAO.buildExplainOutputFromSqlId(sqlId, childNumber, database));
            if ( !env.getParameter("explainusername").equals("") && !env.getParameter("explainpassword").equals("") ) {
              try {
       ...
       myView.setRunTime(System.currentTimeMillis()-startTime);
...
//        }
    ...
    private void setDatabaseList(String currentPool, String currentDb) throws Exception {

      try {
      	
      	if ( currentDb != null )
      	  currentDb = StringUtility.replace(currentDb, ".ORACLE.CFX", "").toLowerCase();
      	
    		Tracer.log("currentPool: " + currentPool + " currentDb: " + currentDb, Tracer.DEBUG,OBJECT_NAME);
          
        ArrayList dbList = ConnectionConfigurationHolder.getPoolList();       
        ArrayList dbSelectList = new ArrayList();
        ArrayList dbShortList = new ArrayList();
        
        ArrayList dbs = new ArrayList();
        String poolToAdd = null;
        ...
        for ( int i=0; i<dbList.size(); i++ ) {
          ...
          if ( !dbs.contains(cfg.getDatabaseName() )) {
          	if ( cfg.getLastStatus() == DbState.UP || cfg.getLastStatus() == DbState.UNKNOWN ) {
              // If the db we are adding is of the current instance, then add the current instance
          		// instead of the first occurance of the databse in the list.
          		if ( currentDb != null && dbName.equals(currentDb) ) {
          			poolToAdd = currentPool;
          		} else {
          			poolToAdd = poolName;
          		}
          		Tracer.log("Added Pool " + poolToAdd, Tracer.DEBUG,OBJECT_NAME);
          	  dbShortList.add(new SelectEntry(poolToAdd, " " + StringUtility.initCap(dbName)));
              dbs.add(cfg.getDatabaseName());
          	}
          }
        ...
        myView.setShortDbList(dbShortList);
    ...
    private void setLookupLists() throws Exception {
    	...
    	// TODO: Put this info into the XML for the actions.xml entries.
      ...
      try {
        ...
        if ( this.action == PowerDbaActions.DB_SESSIONS || 
             ...
             this.actionString.equals("CURRENT_SERVICE_METRICS")
            ) {        
    ...
    private String getTableHtml(String owner, String name, OracleDatabaseConnection database) throws SQLException {
      ...
      try {
        ...
        html.append(ProcessDAO.DIV);
        html.append(ProcessDAO.getPartitions(owner, name, "TABLE", database));
      }
      catch (SQLException e) {
        throw e;
      }
